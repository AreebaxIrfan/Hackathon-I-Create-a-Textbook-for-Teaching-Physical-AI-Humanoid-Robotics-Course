...
- [ ] T001 Create backend project structure per plan.md: backend/app/, backend/app/services/, backend/app/api/, backend/app/middleware/, backend/tests/
- [ ] T002 Initialize Python 3.11+ project with pyproject.toml and requirements.txt (dependencies from research.md)
- [ ] T003 [P] Configure development tools: pylint, black, mypy, pytest configuration files
- [ ] T004 [P] Create .env.example file in backend/ with required environment variables (GEMINI_API_KEY, QDRANT_URL, QDRANT_API_KEY)
- [ ] T005 [P] Create backend/README.md with quickstart instructions from quickstart.md
- [ ] T006 Create backend/app/__init__.py and backend/app/config.py for environment variable management using python-dotenv
...
- [ ] T007 Implement Qdrant client connection in backend/app/services/qdrant_client.py (create collection physical-ai-textbook-v1, 768 dimensions, cosine similarity)
- [ ] T008 [P] Implement embedding generation using Gemini API in backend/app/services/embeddings.py (text-embedding-004 model via OpenAI SDK)
- [ ] T009 [P] Define Pydantic models in backend/app/api/chat.py (ChatRequest, ChatResponse, Source, etc.)
- [ ] T010 [P] Implement content hash generation utility in backend/scripts/ingest_docs.py (SHA-256 for idempotency)
- [ ] T011 Implement MDX file parsing in backend/scripts/ingest_docs.py using python-frontmatter (extract metadata and content)
- [ ] T012 Implement semantic chunking logic in backend/scripts/ingest_docs.py using tiktoken (500-800 tokens, 100-token overlap, code-block preservation)
- [ ] T013 Create FastAPI application entry point in backend/app/main.py with CORS configuration and dependency injection setup
- [ ] T014 [P] Implement structured logging configuration in backend/app/config.py (JSON format, log levels)
- [ ] T015 [P] Implement error handling utilities in backend/app/middleware/error_handler.py (HTTPException with user-friendly messages)
...
- [ ] T016 [P] [US1] Create backend/tests/unit/test_chat_api.py with test for query "What is embodied intelligence?" (expected: retrieves relevant chunks from Chapter 1)
- [ ] T017 [P] [US1] Create backend/tests/unit/test_docs.py to verify API documentation

### Implementation for User Story 1

- [ ] T018 [US1] Implement Qdrant search function in backend/app/services/qdrant_client.py (top 5 semantic search using cosine similarity, return SearchResult objects)
- [ ] T019 [US1] Implement context building from search results in backend/app/services/agent_service.py (construct payload with chunk text, chapter, section, filename)
- [ ] T020 [US1] Implement OpenAI Agents integration in backend/app/services/agent_service.py using Gemini API via OpenAI SDK (pass context + query, generate answer)
- [ ] T021 [US1] Implement `POST /api/chat/` endpoint in backend/app/api/chat.py (accept ChatRequest, embed query, search Qdrant, call agent, return ChatResponse)
- [ ] T022 [US1] Add input validation for query endpoint in backend/app/api/chat.py (max 500 words for query, max 200 words for selected_text)
- [ ] T023 [US1] Add error handling for Qdrant unavailable and Gemini API failures in backend/app/api/chat.py (graceful degradation per edge cases)
- [ ] T024 [US1] Implement health check endpoint `GET /api/health` in backend/app/api/health.py (verify Qdrant and Gemini API connectivity)
- [ ] T025 [US1] Add logging for query processing in backend/app/api/chat.py (sanitized query text, response time, error tracking)
...
- [ ] T026 [P] [US2] Create unit tests for chunking in backend/tests/unit/ (tests for semantic boundaries, code block preservation, token counting accuracy)
- [ ] T027 [P] [US2] Create unit tests for ingestion in backend/tests/unit/ (test for CLI execution)

### Implementation for User Story 2

- [ ] T028 [US2] Implement recursive MDX file scanner in backend/scripts/ingest_docs.py (scan frontend/docs/, filter .mdx and .md files, extract chapter/section from path)
- [ ] T029 [US2] Implement batch embedding generation in backend/app/services/embeddings.py (process multiple chunks efficiently, handle rate limits)
- [ ] T030 [US2] Implement Qdrant upsert logic in backend/app/services/qdrant_client.py (use content hash as point ID for idempotency, store with payload metadata)
- [ ] T031 [US2] Implement CLI ingestion script main function in backend/scripts/ingest_docs.py (orchestrate: scan → parse → chunk → embed → store, handle errors gracefully)
- [ ] T032 [US2] Add progress logging to CLI script in backend/scripts/ingest_docs.py (stdout output showing files processed, chunks created per file)
- [ ] T033 [US2] Add error handling for CLI script in backend/scripts/ingest_docs.py (catch malformed MDX, API failures, DB errors; continue processing; log errors; set non-zero exit code)
- [ ] T034 [US2] Implement incremental update detection in backend/scripts/ingest_docs.py (compare content hashes, only re-embed changed files)
- [ ] T035 [US2] Add CLI script execution via `python backend/scripts/ingest_docs.py`
...
- [ ] T036 [US3] Extend query embedding logic in backend/app/services/embeddings.py to handle optional selected_text (combine query + selected_text for embedding if provided)
- [ ] T037 [US3] Update Qdrant search in backend/app/services/qdrant_client.py to use combined embedding when selected_text is present
- [ ] T038 [US3] Update `POST /api/chat/` endpoint handler in backend/app/api/chat.py to pass selected_text to embedding and retrieval functions
- [ ] T039 [US3] Add validation for selected_text in backend/app/api/chat.py (max 200 words per FR-015)
- [ ] T040 [US3] Update backend/tests/unit/test_chat_api.py to include test case for contextual query with selected_text
